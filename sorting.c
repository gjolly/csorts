...
SORT(merge) {
    ...
    for (int sublist_size = 1; sublist_size <= length / 2; sublist_size *= 2) {
        ...
        for (sli = 0; sli + 1 < sublist_nb / 2; sli++) {
            // Not the last one
            merge_lists(array(2 * sli * sublist_size), sublist_size,
                        array((2 * sli + 1) * sublist_size), sublist_size, size,
                        comp);
        }
        ...
        if (sublist_nb % 2 == 1) {
            ...
            merge_lists(array(2 * sli * sublist_size), sublist_size * 2,
                        array(2 * (sli + 1) * sublist_size),
                        sublist_size + remains, size, comp);
